...
Start by inspecting the changes. Use the deterministic script to collect review context:

```bash
scripts/collect_review_context.sh
```

If on the `main` branch, review staged changes. If on a different branch, review committed and uncommitted changes compared to `main`.

Run two explicit review passes:

1. Architecture pass: module boundaries, design fit, coupling, and scalability.
2. Implementation pass: logic correctness, edge cases, tests, and error handling.
...
Report findings organized by severity, with concrete locations.
...
| Mistake | Correction |
|---------|------------|
| Surface-level review | Dig into logic and trace data flow |
| Ignoring context | Review changes in relation to the system |
| Only finding negatives | Note what is done well |
...
The following script is used to collect deterministic review context before analysis.
